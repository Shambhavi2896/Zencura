...
from model import db, Appointment, Patient, Doctor, User
import logging
import os
...
@celery_app.task(name='backend.tasks.send_appointment_confirmation.send_confirmation_email', bind=True, max_retries=3)
def send_confirmation_email(self, appointment_id):
    ...
    try:
        ...
        with app.app_context():
            ...
            return {'status': 'success', 'email': user.email, 'appointment_id': appointment_id}
            
    except Exception as e:
        logger.error(f"Error sending confirmation email for appointment {appointment_id}: {str(e)}")
