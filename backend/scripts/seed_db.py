...
from model import User, Department, Doctor, Patient, Appointment, Treatment, Payment
    ...
    with app.app_context():
        ...
        for i in range(40):
            ...
            if status == 'Completed':
                ...
                # Create payment record for completed treatment
                payment = Payment(
                    treatment_id=treatment.id,
                    amount=random.choice([500.00, 750.00, 1000.00, 1500.00, 2000.00]),
                    status=random.choice(['Completed', 'Completed', 'Pending'])  # 2/3 completed, 1/3 pending
                )
