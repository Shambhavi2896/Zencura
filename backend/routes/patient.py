...
def patient_dashboard():
    ...
    cancelled = Appointment.query.filter(
        Appointment.patient_id == pat.id, Appointment.status == "Cancelled"
    ).count()
    total = Appointment.query.filter(Appointment.patient_id == pat.id).count()
    upcoming_list = (
...
def book_appointment():
    ...
    if apt_date < date.today():
        return jsonify(msg="Cannot book past dates"), 400
    if apt_date == date.today() and apt_time < datetime.now().time():
        return jsonify(msg="Cannot book past times for today"), 400
    ...
    try:
        ...
        try:
            ...
            send_confirmation_email(appointment.id)
    except Exception:
        pass
...
@cache.cached(timeout=600, key_prefix="patient_profile")
...
def update_profile():
    ...
    if data.get("dob"):
        try:
            ...
            pat.dob = dt_date.fromisoformat(data["dob"])
        except (ValueError, TypeError):
            pass
    db.session.commit()
    from backend.core.cache import cache
    cache.delete("patient_profile")
...
def request_export():
    ...
    try:
        ...
        file_url = generate_patient_export(pat.id)
        return jsonify(msg="Processed sync", task_id="sync", task_status="SUCCESS", task_result=file_url), 200
...
def pay_treatment(id):
    ...
    return (
        jsonify(
            msg="Payment successful via Mock Gateway",
            transaction_id=treatment.payment.transaction_id,
        ),
        200,
    )
