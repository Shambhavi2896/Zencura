"""Search functionality for the hospital management system.

This module provides global search capabilities for doctors, patients,
...
from model import db, Doctor, Patient, User, Appointment
...
def search_doctors():
    """Search for doctors by name, department, or qualification."""
...
@cached_route(timeout=300)  # Cache for 5 minutes
def search_patients():
    """Search for patients by name, contact, or ID."""
    query = request.args.get('q', '').strip().lower()
...
def search_appointments():
    """Search for appointments by status, date, or participant."""
