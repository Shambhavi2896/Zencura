...
from backend.models import db, Payment, Treatment, Appointment, User, Doctor, Patient
...
def update_payment_status(payment_id):
    ...
    db.session.commit()
    return success_response(
        {"id": payment.id, "status": payment.status}, "Payment status updated"
    )
    ...
    return success_response(
        {
            ...
            "monthly_revenue": {"labels": months, "data": revenues},
        }
    )
