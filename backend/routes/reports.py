"""Advanced reporting and analytics module for hospital management."""
...
from model import db, Appointment, Treatment, Payment, Doctor, Patient, Department
...
def analytics_appointments():
    """Get appointment analytics with trends."""
    if not is_admin():
    ...
    .order_by(db.func.count(Appointment.id).desc()).all()
...
def analytics_treatments():
    """Get treatment completion analytics."""
    if not is_admin():
        return error_response('Forbidden', 403)
    ...
    .order_by(db.func.count(Treatment.id).desc()).all()
    ...
    return success_response({
        ...
        'average_treatments_per_doctor': round(sum(treatment_counts) / max(len(doctor_names), 1), 1)
    })
...
def monthly_report(year, month):
    ...
    revenue_data = db.session.query(db.func.sum(Payment.amount)).filter(
    ...
    doc_performance = db.session.query(
        Doctor.full_name,
        Department.name,
    ...
    department_breakdown = db.session.query(
        Department.name,
        db.func.count(Appointment.id).label('appointments'),
        db.func.sum(db.case((Appointment.status == 'Completed', 1), else_=0)).label('completed')
    ).join(Doctor, Doctor.department_id == Department.id)\
     .join(Appointment, Appointment.doctor_id == Doctor.id)\
     .filter(
        Appointment.date >= start_date,
    ...
    recent_billings = db.session.query(
    ...
    total_patients_seen = db.session.query(Patient.id).distinct()\
    ...
    return success_response({
        ...
        'doctor_performance': [
            {'name': name, 'department': department, 'appointments': appointments_count}
        ...
        'department_breakdown': [
            {'name': name, 'appointments': appointments_count, 'completed': completed_count or 0}
...
def dashboard_summary():
    ...
    pending_revenue = db.session.query(db.func.sum(Payment.amount))\
    ...
    return success_response({
        ...
        'pending': {
            'revenue': float(pending_revenue),
            'count': pending_count
        },
        'top_doctors': [
            {'name': d[0], 'appointments': d[1]} for d in top_doctors
        ]
    })
...
def reports_overview():
    ...
    payment_status_rows = db.session.query(
        Payment.status,
        db.func.count(Payment.id).label('count'),
        db.func.sum(Payment.amount).label('amount')
    ).group_by(Payment.status).all()
    ...
    doctor_rows = db.session.query(
     ...
     .filter(Appointment.date >= month_start, Appointment.date <= month_end)\
     .group_by(Doctor.id, Doctor.full_name, Department.name)\
     .order_by(db.func.count(Appointment.id).desc()).limit(6).all()

    recent_payments_rows = db.session.query(
        Payment, Treatment, Appointment, Patient, Doctor
    ).join(Treatment, Payment.treatment_id == Treatment.id)\
     .join(Appointment, Treatment.appointment_id == Appointment.id)\
     .join(Patient, Appointment.patient_id == Patient.id)\
    ...
    for row_date, count, completed in daily_rows:
        trend_dates.append(row_date.strftime('%d %b'))
        trend_appointments.append(count)
        trend_completed.append(completed or 0)
    ...
    return success_response({
        'summary': {
            ...
            'completed': sum(1 for appointment in appointments if appointment.status == 'Completed'),
            'cancelled': sum(1 for appointment in appointments if appointment.status == 'Cancelled'),
            'revenue': float(monthly_revenue),
            'pending_revenue': float(pending_revenue),
            'patients_seen': db.session.query(Patient.id).join(Appointment, Appointment.patient_id == Patient.id)
                .filter(Appointment.date >= month_start, Appointment.date <= month_end).distinct().count(),
