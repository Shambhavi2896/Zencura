const DoctorDashboard = {
    ...
    setup() {
        ...
        const stats = Vue.reactive({
            upcoming: 0, today: 0, completed: 0, cancelled: 0, patients: 0,
            today_schedule: [], chart_labels: [], chart_data: []
        })
        ...
        const treatmentForm = Vue.reactive({
            appointmentId: null, isEdit: false,
            diagnosis: '', prescription: '', notes: '', next_visit: ''
        })
        ...
        const fetchAppointments = async () => {
            try {
                const res = await fetchApi('/api/doctor/appointments?filter=' + aptFilter.value)
        ...
        const updateStatus = async (id, status) => {
            ...
            try {
                const res = await fetchApi('/api/doctor/appointments/' + id + '/status', {
                    method: 'PUT',
        ...
        const fetchPatients = async () => {
            try {
                const res = await fetchApi('/api/doctor/patients?search=' + encodeURIComponent(patientSearch.value))
        ...
        const viewHistory = async (patientId) => {
            ...
            try {
                const res = await fetchApi('/api/doctor/patients/' + patientId + '/history')
                if (res.ok) {
                    const data = await res.json()
        ...
        const openTreatmentModal = async (appointmentId, isEdit = false) => {
            ...
            if (isEdit) {
                // Load existing treatment data from the appointment
                try {
                    // Find appointment in the list to get patient_id, then get from history
                    const apt = appointments.value.find(a => a.id === appointmentId)
                    if (apt) {
                        const res = await fetchApi('/api/doctor/patients/' + apt.patient_id + '/history')
                        if (res.ok) {
                            const data = await res.json()
                            const record = data.records.find(r => r.appointment_id === appointmentId)
        ...
        const saveTreatment = async () => {
            ...
            try {
                ...
                const res = await fetchApi('/api/doctor/appointments/' + treatmentForm.appointmentId + '/treatment', {
                    method,
                    body: JSON.stringify({
        ...
        return {
            activeTab, doctorName, department, stats,
            aptFilter, appointments, fetchAppointments, updateStatus,
            patients, patientSearch, fetchPatients, selectedPatient,
            historyData, viewHistory,
            availabilityText, newAvailability, savingAvailability, saveAvailability,
            treatmentForm, savingTreatment, openTreatmentModal, saveTreatment,
            switchTab, logout
        }
    }
}
