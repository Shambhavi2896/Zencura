const PatientDashboard = {
                                    ...
                                    <td colspan="7" class="p-3">
                                        <div class="bg-white p-3 rounded border" style="font-size:0.85rem;">
                                            ...
                                            <div class="row g-3">
                                                ...
                                                <div class="col-12 mt-3 pt-3 border-top" v-if="apt.treatment.payment">
                                                    <div class="d-flex justify-content-between align-items-center bg-light p-3 rounded">
                                                        <div>
                                                            <span class="d-block text-muted mb-1" style="font-size:0.75rem; text-transform:uppercase;">Invoice / Payment</span>
                                                            <strong style="font-size:1.1rem; color:var(--navy);">$ {{ apt.treatment.payment.amount.toFixed(2) }}</strong>
                                                            <span class="ms-2 badge" :class="apt.treatment.payment.status === 'Paid' ? 'bg-success' : 'bg-warning text-dark'">{{ apt.treatment.payment.status }}</span>
                                                        </div>
                                                        <button v-if="apt.treatment.payment.status === 'Pending'" class="btn btn-sm btn-dark px-3" @click="processPayment(apt.treatment.id)" :disabled="payingTreatment === apt.treatment.id">
                                                            <span v-if="payingTreatment === apt.treatment.id" class="spinner-border spinner-border-sm me-1" style="width:0.8rem; height:0.8rem;"></span>
                                                            Pay Now (Mock Stripe)
                                                        </button>
                                                        <div v-else class="text-success small fw-medium">
                                                            Tx ID: <span class="badge bg-success bg-opacity-10 text-success font-monospace">{{ apt.treatment.payment.transaction_id }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
  ...
  setup() {
    ...
    const payingTreatment = Vue.ref(null);
      ...
      try {
        const res = await fetchApi(
          "/api/patient/doctors?search=" +
            encodeURIComponent(searchDoctors.value) +
            "&department=" +
            filterDept.value
        );
        if (res.ok) {
    ...
    const fetchAppointments = async () => {
      try {
        const res = await fetchApi(
          "/api/patient/appointments?filter=" + aptFilter.value
        );
    ...
    const processPayment = async (treatmentId) => {
      payingTreatment.value = treatmentId;
      try {
        const res = await fetchApi(
          `/api/patient/treatments/${treatmentId}/pay`,
          { method: "POST" }
        );
        const data = await res.json();
        if (res.ok) {
    ...
    const submitBooking = async () => {
      ...
      if (isReschedule.value) {
        url =
          "/api/patient/appointments/" + rescheduleAptId.value + "/reschedule";
        method = "PUT";
      } else {
    ...
    Vue.onMounted(() => {
      appointmentModalInstance = new bootstrap.Modal(
